...
export class FilesRabbitConsumer implements OnModuleInit {
	...
	async onModuleInit() {
		const queuesToSetup = [
			{
				baseQueue: RabbitMainQueues.FILES_USER_DELETED_Q,
		...
		await this.ch.consume(
			RabbitMainQueues.FILES_USER_DELETED_Q,
			async (msg) => {
				if (!msg) return;
				try {
					...
					console.log(`[FILES][RMQ] consumed event - ${RabbitMainQueues.FILES_USER_DELETED_Q}`);
					...
					try {
						...
						await this.filesService.softDeleteUserFiles(evt.userId);
					} catch (e) {
						console.error("[files][RMQ] error creating inbox message", e);
					}
					...
					if (retries < 3) {
						this.ch.sendToQueue(`${RabbitMainQueues.FILES_USER_DELETED_Q}.retry.1m`, msg.content, {
							persistent: true,
						...
						this.ch.publish(RabbitExchanges.APP_DLX, `${RabbitMainQueues.FILES_USER_DELETED_Q}.dlq`, msg.content, {
							persistent: true,
							contentType: APPLICATION_JSON,
