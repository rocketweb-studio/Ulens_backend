...
export class ProfileService {
	...
	async updateProfile(userId: string, dto: ProfileInputDto): Promise<string> {
		const isUsernameExists = await this.prismaProfileCommandRepository.findProfileByUsername(dto.userName);
		if (isUsernameExists) throw new BadRequestRpcException("Username is already taken", "userName");
